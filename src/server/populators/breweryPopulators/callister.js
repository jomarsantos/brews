const url = 'http://www.callisterbrewing.com/#beer-list';
const code = 'callister';
const codeTag = '['+ code + '] ';
...
module.exports = {
  execute: function (callback) {
		console.log(codeTag + '* UPDATE CALLISTER LINEUPS *');
				...
				if (breweries.length > 0) {
					...
					axios.get(url).then(
						function(response) {
							...
							$("h2:contains('ON TAP')").parent().find('p').each(
								function(item) {
									let brew = $(this).text();
									if (brew.indexOf('* no growler fills') !== -1 ||
										brew.indexOf('Last updated') !== -1
									) {
										return;
									}

									// Grab data for brew
									var brewName = brew.trim();;

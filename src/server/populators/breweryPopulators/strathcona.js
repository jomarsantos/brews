const url = 'http://www.strathconabeer.com/draught_beers';
const code = 'strathcona';
const codeTag = '['+ code + '] ';
...
module.exports = {
  execute: function (callback) {
		console.log(codeTag + '* UPDATE STRATHCONA LINEUPS *');
				...
				if (breweries.length > 0) {
					...
					axios.get(url).then(
						function(response) {
							...
							$('.j_beers_draught_text').find('p strong').each(
								function(item) {
									let brew = $(this).text();
									if (brew.indexOf('% IBU') === -1) {
										return;
									}

									brew = brew.split('% IBU')[0];
									let splitText = brew.split(' ');

									// Grab data for brew
									var brewPercentage = splitText[splitText.length - 1].trim();
									var brewName = capitalize(brew.replace(brewPercentage, '')).trim();

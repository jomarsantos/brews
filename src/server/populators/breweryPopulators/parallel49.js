const url = 'http://parallel49brewing.com/tasting_room';
const code = 'parallel49';
const codeTag = '['+ code + '] ';
...
module.exports = {
  execute: function (callback) {
		console.log(codeTag + '* UPDATE PARALLEL 49 LINEUPS *');
					...
					axios.get(url).then(
						function(response) {
							...
							$(".beer-details").each(
								function(item) {
									// Grab data for brew
									var brewName = $(this).find('.beer-name').text().trim();
									var brewSubtitle = $(this).find('.beer-type').text().trim();
									var brewPercentage = $(this).find('.alc-percentage').text().trim();
									if (brewPercentage == '---%') {
										brewPercentage = -1;
									} else {
										brewPercentage = brewPercentage.replace('%', '')
									}

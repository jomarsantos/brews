const url = 'https://steelandoak.ca/';
const code = 'steelandoak';
const codeTag = '['+ code + '] ';
...
module.exports = {
  execute: function (callback) {
		console.log(codeTag + '* UPDATE STEEL & OAK LINEUPS *');
				...
				if (breweries.length > 0) {
					...
					axios.get(url).then(
						function(response) {
							...
							$("#stacks-layer-tasting-room > div > div.stacks-layerset.stacks-layerset-icon.stacks-layerset-columns-3").children().each(
								function(item) {
									// Grab data for brew
									var brewName = $(this).find('h1').text().trim();
									var brewSubtitle = $(this).find('p').first().text().trim();
									var brewPercentage = -1;
									if (brewSubtitle.includes('% ABV')) {
										brewPercentage = brewSubtitle.split('% ABV')[0].trim();
										if (brewSubtitle.includes('IBU /')) {
											brewSubtitle = capitalize(brewSubtitle.split('IBU /')[1].trim());
										} else {
											brewSubtitle = capitalize(brewSubtitle.replace('/', '').split('ABV')[1].trim());
										}
									} else {
										brewSubtitle = capitalize(brewSubtitle);
									}

									// TODO: description available on website
